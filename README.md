...
- 支持 HTTPS WebDAV 连接测试、首次启用、增量同步、接管远端备份，以及在首次引导或空仓库时从远端恢复
...
- 设置页重新查看恢复码、首次启用 WebDAV 和引导内的远端恢复等高风险操作，也会先触发系统验证
...
- 如果远端已有备份且本地为空，提示用户返回首页并重新进入恢复引导
- 如果远端已有备份且本地也有数据，只允许用户明确选择“上传本地覆盖远端”

远端恢复入口规则：

- 首次启动时，可直接在欢迎页选择“从 WebDAV 备份恢复”
- 如果已经完成初始化，但当前仓库仍为空，可在首页空状态重新进入同一套恢复引导
- 设置页不会直接执行远端恢复，避免把恢复流程和日常 WebDAV 配置混在一起，也避免误覆盖当前设备上的记录
